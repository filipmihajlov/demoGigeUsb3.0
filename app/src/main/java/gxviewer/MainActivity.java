...
public class MainActivity extends AppCompatActivity implements SlidingPaneLayout.PanelSlideListener
  ...
  private boolean m_isRecentKeyPressed = false;
    ///< used for divide home key and recent key
  private FloatingActionButton m_saveImageBtn;              ///< save image floating button

  protected Timer m_refreshFpsTimer = null;
    ///< refresh fps and frame information timer
  protected TimerTask m_refreshFpsTimerTask;
    ///< refresh fps and frame information timer task
  ...
  protected void onStart() {
    ...
    m_saveImageBtn.setBackgroundTintList(
      createColorStateList(Color.BLUE, Color.GRAY, Color.BLUE, Color.GRAY));
    ...
    if (!m_threadShareObj.getIsStart()) {
      ...
      try {
        ...
        DeviceManager.getInstance().startUpdateDeviceList(getApplicationContext(),
          ENUM_DEVICE_TIMEOUT);
   ...
   * this method overrides the onUpdateDeviceListFinished method in the
   * OnUpdateDeviceListFinishedListener interface
  ...
  private void startAcquisitionImage() {
    ...
    m_threadShareObj.m_acquisitionImage = new AcquisitionByGetBitmap(m_device,
      m_threadShareObj);           // acquisition by getBitmap interface
    //        m_threadShareObj.m_acquisitionImage = new AcquisitionByGetImageBySurface(m_device,
      //        m_threadShareObj);   // acquisition by getImageBySurface interface
    //        m_threadShareObj.m_acquisitionImage = new AcquisitionByGetRawImage(m_device,
      //        m_threadShareObj);         // acquisition by getRawImage interface
  ...
  private @NonNull
  ImageImproveParam readImageImproveParam(Device dev) {
...
class ThreadShareObj {
  ...
  public Handler m_threadHandler;
    ///< message handler to be used for child thread send message to main thread
  ...
  final ConcurrentLinkedDeque<RawImage> m_processImageLinkedDeque = new ConcurrentLinkedDeque<>();
    ///< acquire image Linked Deque
  final ConcurrentLinkedDeque<ImageInfo> m_displayRunnableImageLinkedDeque
    = new ConcurrentLinkedDeque<>();///< display image Linked Deque
  final ConcurrentLinkedDeque<ImageInfo> m_rawImageLinkedDeque = new ConcurrentLinkedDeque<>();
    ///< raw image linked deque
  ...
  synchronized void clearSerialNumber() {
    m_deviceSN = "";
  }
  ...
  int getSurfaceMaxWidth() {
    return m_surfaceMaxWidth;
  }

  synchronized void setSurfaceMaxWidth(int w) {
    m_surfaceMaxWidth = w;
  }

  int getSurfaceMaxHeight() {
    return m_surfaceMaxHeight;
  }

  synchronized void setSurfaceMaxHeight(int h) {
    m_surfaceMaxHeight = h;
  }

  SaveImageStatus getTryToSaveImage() {
    return m_tryToSaveImage;
  }

  synchronized void setTryToSaveImage(SaveImageStatus save) {
    m_tryToSaveImage = save;
  }
  ...
  synchronized void setOffLineStatus(boolean offLine) {
    m_isOffLine = offLine;
  }
